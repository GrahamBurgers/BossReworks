...
function init_boss()
	...
	EntitySetComponentsWithTagEnabled( entity, "disabled_at_start", true )
	EntitySetComponentsWithTagEnabled( entity, "enabled_at_start", false )
	PhysicsSetStatic( entity, false )
	
	local pos_x, pos_y = EntityGetTransform( entity )

	GamePlaySound( "data/audio/Desktop/event_cues.bank", "event_cues/sampo_pick/create", pos_x, pos_y )
...
function spawn_minion()
	...
	local eid = EntityLoad( "data/entities/animals/boss_centipede/boss_centipede_minion.xml", x, y )
	EntityAddTag(eid, "boss_reworks_boss_rush")
	local dmg = EntityGetFirstComponent(eid, "DamageModelComponent")
	if dmg then
		ComponentSetValue2(dmg, "blood_material", "plasma_fading")
		ComponentSetValue2(dmg, "blood_multiplier", 0)
		ComponentSetValue2(dmg, "ragdoll_material", "blood_fungi")
		ComponentSetValue2(dmg, "materials_damage", false)
	end
